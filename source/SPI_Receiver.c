...
void activarTiempo(void);
void reiniciar (void);
void stateDefault(void);
 ...
 */
uint_8 data [50] = {0xFE,5,ENABLE|GREEN,TURN_ON|GREEN,TURN_OFF|GREEN,TURN_ON|GREEN,TURN_OFF|GREEN};
#if 0
uint_8 data [50] = {0};
#endif
...
int main (void)
{
	...
	vfnPITCallbackRegister1(activarTiempo);
	vfnInitPit();
...
void readingState(void)
{
	...
	if((contador<temporal[eIteraciones]) && (temporal[eIteraciones]<=TOP) && (temporal[eIteraciones]!=0))
	{
		instruccion=(spi)& 0b11110000;
		led=(spi)&0b00001111;

		valCol = valueinarray(led,&validColor[0],sizeof(validColor)/sizeof(uint_8));
		valCom = valueinarray(instruccion,&validCommand[0],sizeof(validCommand)/sizeof(uint_8));

		if((valCol & valCom) && (spi!=*(ptrtemporal-1)))
		{
			*ptrtemporal = spi;
			tmpCount++;
			contador++;
